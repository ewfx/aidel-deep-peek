...
# Folder Paths
SDN_FOLDER = os.path.join(BASE_DIR, "data/ofac_sanctions_data", "SDN")
ALT_FOLDER = os.path.join(BASE_DIR, "data/ofac_sanctions_data", "ALT")
...
def fuzzy_search_data(df, search_term, column_name, threshold=90):
    ...
    matches = process.extract(search_term, df[column_name].dropna().apply(normalize_text), limit=1)
